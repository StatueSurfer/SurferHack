...
/* called when an attack inflicts necrotic damage on the hero (added by StatueSurfer, 12/15/17, credit to aosdict for inspiration). 
Will be placed under the otherwise almost unused AD_DRCO (stupid rabid rats...)*/
void
necrosis(reason, pkiller, fatal)
const char *reason,    /* controls what messages we display */
		   *pkiller;   /* for score+log file if fatal */
int fatal;        /* this is going to be tricky, since both the attack's normal damage and the max hp drain could potentially kill the hero */
//boolean thrown_weapon; /* thrown weapons are less deadly */ <- shouldn't even be needed unless there's an artifact weapon that deals necrotic damage
{
	int i, loss, kprefix = KILLED_BY_AN;
	...
	"blast" has given a "blast of poison gas" message; "poison arrow",
	"poison dart", etc have implicitly given poison messages too... */
	if (strcmp(reason, "blast") && !strstri(reason, "rotting")) {
		boolean plural = (reason[strlen(reason) - 1] == 's') ? 1 : 0;

		/* avoid "The" Orcus's sting was rotten... */
		pline("%s%s %s rots you!",
			isupper((uchar)*reason) ? "" : "The ", reason,
			"");
	}
	/* attack reduces max hp */
	//u.uhpmax -= 4 * rnd(3); or not; could be imbalanced in the other direction
	...
	if (i >= LOW_PM && (mons[i].geno & G_UNIQ)) {
		...
		/*[ does this need a plural check too? ]*/
		kprefix = KILLED_BY;
	} //i'll just leave the above two up there if
	...
	/* attribute loss; drains A_CON and takes some max HP with it */
	loss = (!fatal) ? 1 : d(1, 2); /* since only MC can block this damage, only 1-2 points will be lost */
														/* check that a stat change was made */
	//if (adjattrib(typ, -loss, 1)) //just ignore this, and type the relevant line
	adjattrib(A_CON, -loss, 1);
	pline("Your skin rots away a little!");

...
#define TimedTrouble(P) (((P) && !((P) & ~TIMEOUT)) ? ((P) & TIMEOUT) : 0L)
    ...
    for (val = 0; val < val_limit; val++) {
        ...
        default:
            ...
            break;
			if (obj && (!obj->blessed && !obj->cursed)) {
				curse(obj);
			}
			if (obj && obj->blessed) {
				unbless(obj); //a successful unihorn application degrades its BUC - StatueSurfer, 12/16/17
			}
        }
    }
	...
	if (did_attr) {
		...
		if (obj && obj->blessed) {
			unbless(obj); //see above, StatueSurfer, 12/16/17
		}
	}
    else if (!did_prop)
        pline("Nothing seems to happen.");

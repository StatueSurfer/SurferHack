...
register struct monst *mtmp;
{
    ...
    for (i = 0; i < NATTK; i++) {
        ...
        case AT_ENGL:
            ...
            break;
        case AT_BREA:
            //if (range2) i'm pretty sure this is what blocks breath attacks from being used at range, so out the door it goes - StatueSurfer, 12/19/17
...
register struct attack *mattk;
{
    ...
    case AD_DRLI:
        ...
        break;
	case AD_NECR: //necrotic damage, added by StatueSurfer, 12/16/17
		if (uncancelled && !is_undead(youmonst.data) && !is_demon(youmonst.data)) {
			Sprintf(buf, "%s %s", s_suffix(Monnam(mtmp)),
				mpoisons_subj(mtmp, mattk));
			necrosis(buf, mdat->mname, 30);
		}
		if (is_undead(youmonst.data)) {
			pline("You seem no more rotted than before.");
		}
		if (is_demon(youmonst.data)) {
			pline("Your flesh is unaffected by any rot.");
		}
		else {
			Sprintf(buf, "%s sure has bad breath.", Monnam(mtmp));
		}
		break;
    case AD_LEGS: {
...
struct attack *mattk;
{
    ...
    case AD_STON:
        ...
        if (Reflecting && couldsee(mtmp->mx, mtmp->my)
            ...
            stoned = TRUE;
            killed(mtmp); this block of code makes no sense because Medusa is stoning resistant,
			and shouldn't be able to be turned to stone by her own gaze as a result. - StatueSurfer, 12/24/17 */

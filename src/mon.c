...
boolean via_attack;
{
    if (via_attack && sengr_at("Elbereth", u.ux, u.uy, TRUE)) {
		...
		/* Since this is the logic, then I'm stepping the penalty up to
		   balance things out - StatueSurfer, 12/18/17 
		   The real problem being that since attacking a peaceful turns it
		   hostile right before it dies - even if it dies in one melee hit
		   then there are a rather low number of circumstances in which the 
		   player can actually kill a still-peaceful monster. */
        adjalign(-15);
    ...
    if (mtmp->ispriest) {
        if (p_coaligned(mtmp))
            adjalign(-20); /* very bad */
        else
            adjalign(5);
    } else if (!(u.ualign.type == A_CHAOTIC && uwep->oartifact == ART_STORMBRINGER)) //Takes away the penalty for attacking peacefuls if you're a chaotic wielding Stormbringer
        adjalign(-10); /* attacking peaceful monsters is bad */ /* and it just got a lot worse. StatueSurfer, 12/18/17 */
    if (couldsee(mtmp->mx, mtmp->my)) {

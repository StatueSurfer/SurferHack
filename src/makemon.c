...
register struct monst *mtmp;
{
    ...
    case S_DEMON:
        ...
        case PM_ORCUS:
			...
			/* how about no; Orcus' (by that point) harmless WoD supersedes his spellcasting,
			which would make far more of a challenge (StatueSurfer, 12/16/17) */
            break;
...
register struct monst *mtmp;
{
    ...
    case S_DEMON:
        ...
        if (ptr == &mons[PM_ICE_DEVIL] && !rn2(4)) {
            ...
            (void) mongets(mtmp, WAN_NOTHING);
           /* (void) mongets(mtmp, WAN_FIRE); as part of Asmodeus' degradation to joke boss (but wasn't he already?) - StatueSurfer, 12/18/17*/
        }
        break;
    case S_GNOME:
 ...
 *  
 * Unfortunately, the system is still horrendously unbalanced; the player can
 * wantonly cut down every peaceful monster in the game, but unless they 
 * commit murder, there won't be any lasting consequences due to the fact they
 * will recover alignment from killing hostiles so quickly.
 * New rules:
 *  it's awful to kill peaceful monsters, and super-duper awful to kill always-
 *		peaceful monsters;
 *	it's never bad to kill a hostile monster, but there is no reward if the monster
 *		is coaligned.
 * - StatueSurfer, 12/17/17
 * Another problem is that when you attack a peaceful, they are turned hostile,
 * allowing you dodge the penalty for killing them. So a sizable punishment should
 * be placed on the player for merely _attacking_ a peaceful in order to be
 * effective. I've implemented this over in mon.c, so I will now be reducing the
 * penalty for killing a peaceful.
 * 
 * As a side note, if implemented properly, this could nerf the Neutral
 * alignment, as most monsters above Hell are neutral.
 */
void
set_malign(mtmp)
struct monst *mtmp;
{
    ...
    } else if (always_peaceful(mtmp->data) && !(u.ualign.type = A_CHAOTIC && uwep->oartifact == ART_STORMBRINGER)) { //part of the exception for chaotics with Stormbringer - StatueSurfer, 12/19/17
        int absmal = abs(mal);
        if (mtmp->mpeaceful)
            mtmp->malign = -3 * max(5, absmal);
        else
            mtmp->malign = 0; /* renegade */ /* well, if it was always peaceful, then presumably player action made it hostile, so... */
    } else if (always_hostile(mtmp->data)) {
        int absmal = abs(mal);
        if (coaligned)
            mtmp->malign = 0;
        else
            mtmp->malign = 0; //after some discussion on IRC, I determined that is the only way to make alignment actually matter - StatueSurfer, 12/20/17
    } else if (coaligned) {
        int absmal = abs(mal);
		if (mtmp->mpeaceful && !(u.ualign.type = A_CHAOTIC && uwep->oartifact == ART_STORMBRINGER)) //see above exception
			mtmp->malign = -5; //reduced from -20 since attacking a peaceful now incurs a steep penalty
		else /* renegade */
			mtmp->malign = /*max(3, absmal); nope it's still coaligned and therefore no alignment boost for you*/ 0;

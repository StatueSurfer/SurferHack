    ...
    if (lined_up(mtmp)) {
        ...
        if (!mtmp->mspec_used && rn2(3)) {
            if ((typ >= AD_MAGM) && (typ <= AD_ACID)) {
                 ...
                 */
				if (!rn2(3))
					mtmp->mspec_used = rn2(4); /*10 + rn2(20); this timeout feels a little too long to me. I mean, in most depictions, you see dragons just raining destruction everything,
									   not using it once every lunar cycle. Timeout reduced to d4 turns. - StatueSurfer, 12/19/17*/

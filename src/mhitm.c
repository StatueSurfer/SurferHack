...
register struct attack *mattk;
{
	...
	case AD_NECR:
		if (!cancelled) {
			if (vis && canspotmon(magr))
				pline("%s %s rots %s!", s_suffix(Monnam(magr)),
					mpoisons_subj(magr, mattk), mon_nam(mdef));
				mdef->mhpmax -= rnd(6); 
			...
			if (is_undead(mdef->data)) {
				if (vis && canspotmon(mdef) && canspotmon(magr))
					pline("%s doesn't seem anymore rotted than before.",
						mon_nam(mdef));
			}
			if (is_demon(mdef->data)) {
				if (vis && canspotmon(mdef) && canspotmon(magr))
					pline("%s flesh doesn't seem harmed.",
						s_suffix(Monnam(mdef)));
			}
			else {
				if (!is_undead(mdef->data) && !is_demon(mdef->data))
				{
					mdef->mhpmax -= rnd(6);
					if (mdef->mhp > mdef->mhpmax)
					{
			...
			if (vis && canspotmon(magr) && canspotmon(mdef)) {
				pline("%s recoils from %s bad breath!", mon_nam(mdef),

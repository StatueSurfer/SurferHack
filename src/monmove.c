...
struct monst *mtmp;
{
	...
	/* However, demon lords/princes are now immune to scare monster.
	 * It is also ineffective on the planes.
	 * -StatueSurfer, 12/17/17 */
    if (sobj_at(SCR_SCARE_MONSTER, x, y) && !is_dprince(mtmp->data) && !is_dlord(mtmp->data)
		&& !In_endgame(&u.uz))
        return TRUE;

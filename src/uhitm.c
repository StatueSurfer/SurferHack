	...
	case AD_NECR:
		if (canspotmon(mdef)) {
			if (!is_undead(mdef->data) && !is_demon(mdef->data)) {
				pline("%s flesh rots!", s_suffix(Monnam(mdef)));
				mdef->mhpmax -= rnd(6);
			...
			if (is_undead(mdef->data)) {
				pline("%s seems no more rotted than before.", mon_nam(mdef));
			}
			if (is_demon(mdef->data)) {
				pline("%s flesh is unaffected by rot.", s_suffix(Monnam(mdef)));
			}
		}
		else {
			if (!is_undead(mdef->data) && !is_demon(mdef->data)) {
				pline("%s shudders!", mon_nam(mdef));
				mdef->mhpmax -= rnd(6);
				if (mdef->mhp > mdef->mhpmax) {
					mdef->mhp = mdef->mhpmax;
				}
